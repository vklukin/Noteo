    ...
    handleContextMenuButtonClick = (click) => {
        switch (click) {
            case 'remove':
                ...
                break;
            default:
                return toast.info('При выполнении действия, произошла неизвестная ошибка!');
        }
    }

    //post query for remove note
    RemoveNote = () => {
        PostRemoveNoteQuery(this.props.id, this.cardRef.current)
            ...
            <>
                <div className="card" data-id={this.props.id} ref={this.cardRef}>
                    ...
                    <div className="manual-items">
                        <button className="card-button" onClick={this.handleContextMenu} ref={this.contextButtonRef}>
                            <ThreeDots/>
                        </button>
                    </div>
                    {this.state.showContext && <div className="context-div" ref={this.contextMenuRef}>
                        <button className="context-button context-button-remove"
                                onClick={() => this.handleContextMenuButtonClick('remove')}>Удалить заметку
                        </button>
                    </div>}
                </div>
                {this.state.modal && <div className="remove-note-modal">
                    <div className="note-modal-wrapper">
                        <p>Это действие является необратимым. Хотите ли вы продолжить?</p>
                        <div className="note-modal-wrapper-controllers">
                            <button className="btn-answer cancel"
                                    onClick={() => {
                                        this.setState({modal: false,})
                                    }}>Отменить
                            </button>
                            <button className="btn-answer accept"
                                    onClick={() => this.RemoveNote()}>Подтвердить
                            </button>
                        </div>
                    </div>
                </div>}
